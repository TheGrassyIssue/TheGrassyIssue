#!/usr/bin/env python3
"""localize-axxa.py — pull the 18 AXXA product shots local. 20 Sept 2026.
...
has a width under the cap and a height way over it. AXXA shoot on-model in mixed orientations.
...
def main(apply_):
    ...
    for slug, name, cat, price, was, avail, total, url, img, spec in rows:
        dest = OUT / f"{slug}.jpg"
        if dest.exists():
            skipped.append(slug)
            continue
    ...
    if apply_:
        ...
        print(f"\n  {len(want)}/{len(rows)} images local in images/axxa/")
    else:
        print("\n  dry run — pass --apply")
